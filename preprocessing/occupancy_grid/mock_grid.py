...
def create_mock_grid(num_x_cells: int, num_y_cells:int, cell_size: float, occupancy_percentage: int) -> OccupancyGrid:
    """
    ...
    available_cells = np.asarray([(x, y) for x in range(num_x_cells) for y in range(num_y_cells)])

    occupied_cells = np.random.choice(grid.get_x_cells() * grid.get_y_cells(), size=int(occupancy_percentage * grid.get_x_cells() * grid.get_y_cells() / 100), replace=False)
...
# define main
def main():
    grid = create_mock_grid(num_x_cells=100, num_y_cells=100, cell_size=0.1, occupancy_percentage=5)
    grid.plot()
    plt.show()
